...
-- Script de migration robuste avec gestion des tables existantes
-- =====================================================
...
DROP TABLE IF EXISTS speed_networking_sessions CASCADE;
DROP TABLE IF EXISTS networking_rooms CASCADE;
...
CREATE INDEX idx_mini_sites_exhibitor ON mini_sites(exhibitor_id);
CREATE INDEX idx_mini_sites_slug ON mini_sites(slug);
CREATE INDEX idx_mini_sites_published ON mini_sites(published);
...
CREATE INDEX idx_site_templates_popularity ON site_templates(popularity DESC);
...
CREATE INDEX idx_site_images_exhibitor ON site_images(exhibitor_id);
...
CREATE INDEX idx_networking_timestamp ON networking_interactions(timestamp DESC);
...
CREATE INDEX idx_match_scores_user1 ON match_scores(user_id_1);
CREATE INDEX idx_match_scores_user2 ON match_scores(user_id_2);
...
CREATE INDEX idx_speed_networking_event ON speed_networking_sessions(event_id);
CREATE INDEX idx_speed_networking_status ON speed_networking_sessions(status);
CREATE INDEX idx_speed_networking_start_time ON speed_networking_sessions(start_time);
...
CREATE INDEX idx_networking_rooms_event ON networking_rooms(event_id);
CREATE INDEX idx_networking_rooms_sector ON networking_rooms(sector);
CREATE INDEX idx_networking_rooms_status ON networking_rooms(status);

-- =====================================================
-- ÉTAPE 3: STORAGE BUCKET
-- =====================================================
...
-- =====================================================
-- ÉTAPE 4: ROW LEVEL SECURITY (RLS) POLICIES
-- =====================================================
...
BEGIN
  ...
  RAISE NOTICE '';
  RAISE NOTICE '🚀 Prochaine étape : Seedez les templates';
  RAISE NOTICE '   npm run seed:templates';
  RAISE NOTICE '================================================';
END $$;
