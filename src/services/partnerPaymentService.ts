...
export async function getPartnerBankTransferRequests(userId: string): Promise<any[]> {
  try {
    ...
    const { data, error } = await supabase
      .from('payment_requests')
      .select('id, user_id, requested_level, amount, currency, payment_method, transfer_reference, status, transfer_proof_url, admin_notes, approved_at, rejected_at, created_at, validated_by, validated_at, validation_notes')
      .eq('user_id', userId)
