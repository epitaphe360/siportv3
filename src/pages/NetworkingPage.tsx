...
export default function NetworkingPage() {
  ...
  React.useEffect(() => {
    if (isAuthenticated && user) {
      ...
      // Vérifier si on doit générer automatiquement les recommandations
      ...
      if (shouldGenerate && !hasAutoGenerated) {
        ...
        // Générer les recommandations après un court délai
        setTimeout(async () => {
          toast.loading('🤖 Génération de vos recommandations IA personnalisées...');
          try {
            await generateRecommendations(user.id);
            toast.dismiss();
            toast.success('✨ Vos recommandations sont prêtes ! Découvrez vos meilleurs matches.', {
              duration: 4000,
              icon: '🎯'
            });
          } catch (error) {
  ...
  }, [isAuthenticated, user, fetchRecommendations, hasAutoGenerated, generateRecommendations]);
                  ...
                  {connections.map((connectionId) => {
                    ...
                    return (
                      <Card key={connectionId} className="p-6 bg-white hover:shadow-xl transition-all duration-300 border-slate-100 rounded-[2rem] flex items-center gap-6">
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className="font-black text-slate-900 truncate">{displayName}</h4>
                          <p className="text-xs font-bold text-indigo-600 uppercase tracking-wider truncate">{position}</p>
